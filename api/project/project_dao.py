...
def load_projects_by_userid(userid):
    g.db.cs.execute("SELECT * FROM projects WHERE user_id=?", (userid,))
    mylogger.debug(f"查询所有的项目 {userid=}")
...
def drop_project(project_id) -> None:
