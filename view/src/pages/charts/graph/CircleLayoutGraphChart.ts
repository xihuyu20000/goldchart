export async function CircleLayoutGraphChart() {
  const resp = await $chart({ chart_id: "/graph/CircleLayoutGraphChart" });
  let graph = {
    nodes: [
      {
        id: "0",
  ...
  option.series[0].categories = resp.data.categories;
